...
high <- hd8 %>% dplyr::filter(hd8$age > 14.99)
middle <- hd8 %>% dplyr::filter(between(hd8$age, 11.99, 14.99))
elementary <- hd8 %>% dplyr::filter(hd8$age < 11.99)
...
demos <- demos %>%
    rename("clinicallycrap" = "bmi")

# remove specific variables from a df
high <- dplyr::select(high,-c(pa, cog_fluid)) # removes two vars var from tibble
